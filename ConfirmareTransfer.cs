...
namespace BibliotecaN
{
    public partial class ConfirmareTransfer : Form
    {
        ...
        private void buttonDa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (var db = new BibliotecaEntities())
                {
                    ...
                    if (imprumut.Stare_Imprumut == 0)
                    {
                        inreg.Accept_destinatar = 3;
                        root.listView1.Items.Clear();
                        db.SaveChanges();
                        root.UpdateListView();

                        this.Close();
                        throw new Exception("Cartea este returnata!");
        ...
        private void ConfirmareTransfer_Load(object sender, EventArgs e)
        {
